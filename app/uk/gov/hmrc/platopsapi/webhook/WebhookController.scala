...
class WebhookController @Inject()(
  ...
  repository      : GithubRequestsQueueRepository,
  cc              : ControllerComponents
)(implicit
  ec: ExecutionContext
) extends BackendController(cc) with Logging:

  val actorRef = ActorSystem(WorkItemActor(repository, servicesConfig, webhookConnector), "work-item-actor")
    ...
    Action.async(
        ...
        Accumulator(Sink.fold[ByteString, ByteString](ByteString.empty)(_ ++ _))
          ...
          .map: payloadAsString =>
            rh.headers.get("X-Hub-Signature-256") match
              //if WebhookController.isSignatureValid(payloadAsString, platopApiWebhookSecretKey, sig)
              case Some(sig)  if sig == "test"
      ...
      request
        ...
        .map(WebhookEvent.parse) match
          case Some(Right(event)) => logger.info(s"Pushing webhook ${event.asString} to github requests queue")
                                     eventMap(event)
                                      .traverse: url =>
                                         repository
                                           .pushNew(
                                             GithubWebhookRequest(
                                               xGitHubEventHeader = event
                                             , targetUrl          = url.toString
                                             , payload            = request.body
                                             )
                                           )
                                      .map: _ =>
                                        //actorRef ! ProcessWorkItems // tell Actor to process work items
